...
void ArrayList::doubleArray(int capacity)
{
    int *ralloc=new int[capacity];
...
void ArrayList::halfArray(int capacity)
{
    int *dalloc=new int[capacity];
    for(int idx=0; idx<lastIndex; ++idx)
        arr_ptr[idx]=arr_ptr[idx+1];
    delete []arr_ptr;
    arr_ptr=dalloc;
...
{
    if(index_location>-1 && index_location<=lastIndex+1)
    {
        if(lastIndex==capacity-1)
            doubleArray(capacity*2);
